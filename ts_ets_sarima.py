# -*- coding: utf-8 -*-
"""
Created on Mon Nov 25 18:04:55 2019
...
def exp_smoothing_forecast(train, test, config):
    ...
    #rmse1 = np.sqrt(mean_squared_error(train, yhat1))
    rmse2 = np.sqrt(mean_squared_error(test, yhat2))
    return (rmse2)
...
# final model
y_hat_avg = test.copy()
model = SARIMAX(train, order=(2,0,2), seasonal_order=(1,1,1,6), trend='c', enforce_stationarity=False, enforce_invertibility=False)
model_fit = model.fit(disp=False)
